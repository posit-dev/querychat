...
from ._artifact_types import ARTIFACT_FORMATS, LANGUAGES
...
def build_modal_ui(
    ...
    return ui.modal(
        ...
        size="l",
        easy_close=True,
        id=ns("artifact_modal_root"),
        class_="querychat-artifact-modal",
    )
...
def build_type_selector() -> TagList:
    pills = []
    for i, (format_id, artifact_format) in enumerate(ARTIFACT_FORMATS.items()):
        active_class = " active" if i == 0 else ""
        label = TagList(bs_icon(artifact_format.icon), " ", artifact_format.label)
        pills.append(
            tags.button(
                label,
                class_=f"querychat-artifact-type-pill{active_class}",
                type="button",
                data_artifact_type=format_id,
                data_languages=",".join(artifact_format.supported_languages),
            )
        )
    pills.append(
        tags.button(
