...
In some Shiny applications, you may need to create database connections per-user rather than globally. For example, when connections depend on session-specific authentication or user credentials. The deferred data source pattern supports this by allowing `data_source = NULL` at initialization:
...
- **`table_name` is required** - it cannot be inferred from `NULL`
