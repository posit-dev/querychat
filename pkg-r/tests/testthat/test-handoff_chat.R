...
describe("HandoffChat$ask()", {
  ...
  it("does not apply a max_tokens override", {
    chat <- MockHandoffChat$new(structured_result = list(answer = "42"))
    type <- ellmer::type_object(answer = ellmer::type_string())

    sync_promise(HandoffChat$new(chat)$ask("recommend", type))

    expect_null(chat$requests()[[1]]$max_tokens)
  })
})
...
describe("HandoffChat$stream()", {
  ...
  it("applies HANDOFF_MAX_TOKENS to the forked chat", {
    parsed <- new_mock_handoff_result("print(1)")
    chat <- MockHandoffChat$new(
      stream_chunks = paste0(
        '{"source":"print(1)","language":"r",',
    ...
    expect_equal(chat$requests()[[1]]$max_tokens, HANDOFF_MAX_TOKENS)
