...
- Keep visualizations simple and readable: limit facets to 4-6 panels, prefer fewer series/legend entries, and avoid dense text annotations
- For large datasets, aggregate in the SQL portion before visualizing — avoid plotting raw rows when the table has many thousands of records. Use GROUP BY, sampling, or binning to keep charts readable and responsive
