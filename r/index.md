...
- “Show only penguins that are not species Gentoo and have a bill length
  greater than 50mm.”
- “Show only blue states with an incidence rate greater than 100 per
  100,000 people.”
...
forcing it to write SQL queries instead. See the section below on [“How
it works”](#how-it-works) for more.
...
First, you’ll need an OpenAI API key. See the [instructions from
...
querychat’s natural language chat experience is powered by LLMs. You may
...
The small open source models (8B and below) we’ve tested have fared
extremely poorly. Sorry. 🤷
...
- **Reliability:** Today’s LLMs are excellent at writing SQL, but bad at
  direct calculation.
...
1.  The basic set of behaviors the LLM must follow in order for
    querychat to work properly. (See `inst/prompt/prompt.md` if you’re
...
And that’s all the LLM will know about your data. The actual data does
...
surprising amount of sense out of the data. But if your data frame’s
columns are `x`, `V1`, `value`, etc., then the model will need to be
given more background info–just like a human would.
...
**Warning:** It is not 100% guaranteed that the LLM will always—or in
many cases, ever—obey your instructions, and it can be difficult to
