...
def test_server_resolves_history_and_warns_on_explicit_enable_bookmarking(monkeypatch):
    ...
    with (
        ...
        # Explicit enable_bookmarking=True/False: warns, but no longer has any
        # effect on its own -- mod_server() no longer takes it at all.
        with pytest.warns(FutureWarning, match="history"):
            qc.server(enable_bookmarking=True)
        assert "legacy_enable_bookmarking" not in captured
