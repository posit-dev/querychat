from querychat._handoff_gallery import VizGalleryItem
from querychat._handoff_modal import (
...
class TestTypeSelectorLanguages:
    def test_type_selector_reads_languages_from_registry(self):
        ...
        assert 'data-handoff-type="marimo-notebook"' in html
        assert 'data-languages="python"' in html
        assert 'data-handoff-type="shiny-app"' in html
        assert 'data-languages="python,r"' in html

def test_modal_body_has_namespaced_handoff_root():
    html = str(build_modal_ui(ns, []))
    assert 'id="ns-handoff_modal_root"' in html
    assert 'class="modal-body querychat-handoff-modal"' in html
