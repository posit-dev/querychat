...
5. [Pins](https://rstudio.github.io/pins-python/) boards.
6. A custom [DataSource](reference/types.DataSource.qmd) interface/protocol.
...
You can pass a [pins](https://rstudio.github.io/pins-python/) board directly to `QueryChat` with the pin name as `table_name`:

```shell
pip install querychat[pins]
```

```{.python filename="pins-app.py"}
import pins
from querychat import QueryChat
...
The pin is read and loaded into an in-memory DuckDB database, the same as data frames. For parquet, CSV, JSON, and arrow pins, the cached files go straight into DuckDB (or through polars for arrow) without Python deserialization. Other pin types go through `pin_read()` first.

If the pin has a title, description, or tags, querychat uses them as the default `data_description`, which you can override.

Pin names with special characters (like `"user.name/my_pin"`) are sanitized into valid SQL table names. To control the table name yourself, use `PinSource`:

```{.python}
from querychat.types import PinSource

ps = PinSource(board, "user.name/my_pin", table_name="my_data")
qc = QueryChat(ps)
```

### Lazy queries with pins

By default, the full dataset is materialized into DuckDB. For large parquet pins, you can skip that step by reading the pin files yourself and passing a Polars LazyFrame:

```{.python filename="pins-lazy-app.py"}
import pins
import polars as pl
from querychat import QueryChat
...
The pin files are still downloaded to a local cache --- `pin_download()` always fetches them. But rather than loading everything into memory, Polars reads the parquet file lazily. This is not the same as a database-backed source, where data never leaves the server.
