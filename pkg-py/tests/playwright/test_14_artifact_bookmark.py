...
that the revise step explicitly re-triggers bookmarking. The conversation
history must then reopen the bookmark that contains the latest revision.
"""
...
class TestArtifactBookmarkRestore(ArtifactModalActions):
    ...
    def test_revision_updates_history_bookmark_and_restores_latest(self) -> None:
        ...
        # Start a new conversation, then reopen the revised conversation through
        # shinychat history. This path uses record.bookmark_state_id, which was
        # stale before the fix.
        history_trigger = self.page.locator(".shiny-chat-history-trigger")
        history_trigger.click()
        history_new = self.page.locator(".shiny-chat-history-new")
        expect(history_new).to_be_visible(timeout=5_000)
        history_new.click()
        self.page.wait_for_function(
        ...
        self.page.wait_for_selector("shiny-chat-container", timeout=30_000)

        history_trigger = self.page.locator(".shiny-chat-history-trigger")
        history_trigger.click()
        history_item = self.page.locator(".shiny-chat-history-item-select").first
        expect(history_item).to_be_visible(timeout=30_000)
        new_conversation_url = self.page.url
        history_item.click()
        self.page.wait_for_function(
            "(previous) => window.location.href !== previous",
            arg=new_conversation_url,
            timeout=30_000,
        )
        assert self.page.url == bookmark_url
