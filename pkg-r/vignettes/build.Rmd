...
## Chat-first page {#chat-first-page}
...
Since `$page()` owns the entire page layout, don't wrap it in another page container (e.g., `page_sidebar()`). If you need the chat embedded alongside other content in a custom layout, use `$sidebar()` or `$ui()` instead.
...
A data source passed to `$server()` belongs to that session: other sessions never see it, it does not change the tables registered on the `QueryChat` object, and querychat cleans up any connection it created for it when the session ends. Connections you open yourself are yours to close; `session$onSessionEnded()` is a good place.
...
If your data lives in a DBI connection, use `$add_tables()` to register all tables in a single call:
