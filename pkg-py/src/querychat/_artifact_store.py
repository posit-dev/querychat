...
lives in `_artifact_orchestrator.py`.
"""
...
# artifacts can't grow memory without bound. The least-recently-used artifact is
# evicted past this; reopening an evicted artifact's chat pill simply no-ops.
...
class ArtifactStore:
    ...
    def remember(self, state: ArtifactState) -> list[ArtifactState]:
        """Store an artifact, evicting the least-recently-used past the cap."""
        removed: list[ArtifactState] = []
        replaced = self._items.pop(state.artifact_id, None)
    ...
    def replace(self, states: list[ArtifactState]) -> list[ArtifactState]:
        """Replace all artifacts while preserving the supplied LRU order."""
