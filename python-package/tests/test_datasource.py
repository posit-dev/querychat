...
def test_db_engine():
    ...
    test_data = [
        ...
        (4, "Diana", 28, 70000.25, True, "2023-05-10", "C", 89.8, "Mid-level developer"),
...
def test_get_schema_categorical_values(test_db_engine):
    ...
    assert "'A'" in schema and "'B'" in schema and "'C'" in schema
...
def test_get_schema_non_categorical_text(test_db_engine):
    ...
    description_line_idx = next(i for i, line in enumerate(lines) if "- description (TEXT)" in line)
...
def test_get_schema_table_structure(test_db_engine):
    ...
    expected_columns = ["id", "name", "age", "salary", "is_active", "join_date", "category", "score", "description"]
    for col in expected_columns:
        assert any(f"- {col} (" in line for line in lines), f"Column {col} not found in schema"
