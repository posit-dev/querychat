...
Under the hood, `querychat` is powered by
[ellmer](https://ellmer.tidyverse.org/), a library for building
chat-based applications with large language models (LLMs). `ellmer`
supports a wide range of LLM providers – [see
here](https://ellmer.tidyverse.org/reference/index.html#chat-constructors)
for a full list.
...
To use a particular model, pass a `"{provider}/{model}"` string to the
`client` parameter, which gets passed along to
...
And, if you’d like to effectively set a new default model, you can use
the `querychat.client` R option or the `QUERYCHAT_CLIENT` environment
variable.
...
Note that it can also be useful to pass a full `Chat` object to the
`client` parameter for more advanced use cases (e.g., custom parameters,
tools, etc):
...
Most models require an API key or some other form of authentication. See
the reference page for the relevant model provider (e.g.,
...
variables. Common practice is to use an `.Renviron` file to manage these
variables. For example, for
[`chat_openai()`](https://ellmer.tidyverse.org/reference/chat_openai.html),
you might add to your `.Renviron` file:
...
In theory, you could use any model that has tool calling support, but we
currently recommend (as of November 2025):

- GPT-4.1 (the default)
- Claude 4.5 Sonnet
- Google Gemini 3.0

In our testing, we’ve found that those models strike a good balance
...
`gpt-oss:20b`), but even if you have the compute to run the largest
models, they still tend to lag behind the cloud-hosted options in terms
...
`chat_bedrock_anthropic()`) to the `client` parameter. See the [ellmer
docs](https://ellmer.tidyverse.org/reference/index.html#chat-constructors)
for more details.
