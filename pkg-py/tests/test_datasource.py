...
def test_get_schema_non_categorical_text(test_db_engine):
    ...
    lines = schema.split("\n")
    name_line_idx = next(
        i for i, line in enumerate(lines) if "- name (TEXT)" in line
    )
...
def test_get_schema_empty_result_handling(test_db_engine):
    ...
    with engine.connect() as connection:
        ...
        connection.execute(
            text("CREATE TABLE empty_table (id INTEGER, name TEXT)")
        )
        connection.commit()
...
def test_invalid_table_name():
    ...
    with pytest.raises(
        ValueError, match="Table 'nonexistent' not found in database"
    ):
